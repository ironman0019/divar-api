...
@section('title', 'افزودن مقدار جدید')

@section('content')
<!-- Create Category Value Content -->
<main class="p-4 lg:p-6">
    <!-- Header -->
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6">
        <div>
            <h1 class="text-yellow-primary font-bold text-xl lg:text-2xl mb-2">افزودن مقدار جدید</h1>
            <p class="text-gray-400 text-sm lg:text-base">ایجاد مقدار جدید برای ویژگی</p>
        </div>
    ...
    <div class="bg-dark-secondary rounded-xl border border-yellow-primary/20 p-6">
        <form action="{{ route('admin.categories.values.store') }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="category_attribute_id" 
                        ...
                        @foreach($attributes as $attr)
                            <option value="{{ $attr->id }}" {{ old('category_attribute_id', request('category_attribute_id')) == $attr->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select id="type" 
                            name="type"
                            class="w-full bg-dark-tertiary border border-gray-600 rounded-lg px-4 py-3 text-gray-300 focus:border-yellow-primary focus:ring-1 focus:ring-yellow-primary focus:outline-none transition-colors duration-200">
                        <option value="0" {{ old('type', '0') == '0' ? 'selected' : '' }}>متن</option>
                        <option value="1" {{ old('type') == '1' ? 'selected' : '' }}>عدد</option>
                        <option value="2" {{ old('type') == '2' ? 'selected' : '' }}>انتخاب</option>
                        <option value="3" {{ old('type') == '3' ? 'selected' : '' }}>چند انتخابی</option>
                    </select>
                    @error('type')
                        <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
                    @enderror
