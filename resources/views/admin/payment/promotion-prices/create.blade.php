...
@section('title', 'افزودن تعرفه تبلیغ')

@section('content')
<main class="p-4 lg:p-6">
    <div class="mb-6">
        <h1 class="text-yellow-primary font-bold text-xl lg:text-2xl mb-2">افزودن تعرفه تبلیغ</h1>
    ...
    <div class="bg-dark-secondary rounded-xl border border-yellow-primary/20 p-6">
        <form action="{{ route('admin.payment.promotion-prices.store') }}" method="POST">
            @csrf
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="type" class="w-full bg-dark-tertiary border border-gray-600 rounded-lg px-4 py-3 text-gray-300">
                        <option value="ladder" @selected(old('type') === 'ladder')>نردبان</option>
                        <option value="special" @selected(old('type') === 'special')>ویژه</option>
                    </select>
                ...
                <div class="flex items-end">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', true)) class="w-4 h-4 mr-3">
                        <span class="text-gray-300">فعال</span>
                    </label>
                </div>
            </div>
            <div class="mt-8 pt-6 border-t border-gray-700">
                <button type="submit" class="bg-yellow-primary text-dark-primary px-6 py-3 rounded-lg">ایجاد</button>
            </div>
        </form>
    </div>
</main>
